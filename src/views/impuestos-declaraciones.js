...
export async function impuestosDeclaracionesView() {
  ...
  function renderD150Replica(resultDiv, calc, ctx) {
    ...
    const totalVentasBruto = calc.totalVentasNeto + calc.debitoFiscal;
    const totalVentasBase = calc.totalVentasNeto;
    const totalVentasIVA = calc.debitoFiscal;

    const totalComprasBruto = Object.values(compras).reduce((s, x) => s + (x.total || 0), 0);
    const totalImpuestoSoportado = Object.values(compras).reduce((s, x) => s + (x.iva || 0), 0);
    const comprasConIVA = Object.entries(compras).filter(([t]) => Number(t) > 0).reduce((s, [, x]) => s + (x.total || 0), 0);
    const comprasSinIVA = (compras[0] || compras['0'])?.total || 0;
    ...
    resultDiv.innerHTML = `<style>
      ...
      .d150-resumen{width:280px;flex-shrink:0;background:#fff;border-radius:12px;padding:18px;box-shadow:0 1px 3px rgba(0,0,0,.05)}
      .d150-resumen h4{font-size:14px;font-weight:700;color:#2c2c54;margin:0 0 14px}
