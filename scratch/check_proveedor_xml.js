...
const options = {
  ...
  path: `/rest/v1/fiscal_facturas?select=*&proveedor=eq.${encodeURIComponent('Proveedor XML')}`,
  method: 'GET',
  headers: {
...
const req = https.request(options, (res) => {
  ...
  res.on('end', () => {
    try {
      ...
      console.log('Count of Proveedor XML records:', data.length);
      console.log('Sample records:', JSON.stringify(data.slice(0, 10), null, 2));
    } catch(e) {
